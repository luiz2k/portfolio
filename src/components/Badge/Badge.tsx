...
export default function Badge({ title, imageUrl, children }: BadgeProps) {
  return (
    ...
    >
      ...
      {!children && (
        <span className="absolute -top-8 left-1/2 -translate-x-1/2 rounded bg-color-4 p-1 text-xs text-white opacity-0 duration-150 ease-in-out group-hover:-top-6 group-hover:opacity-100 dark:bg-color-2 dark:text-black">
          {title}
